<template>
  <div style="padding: 0 5px">
    ...
    <van-list v-for="(item, i) in commentsList" :key="i" finished>
      <div class="title">
        <span>第{{i+1}}楼</span>
  ...
  methods: {
    async getComments() {
      const { data: res } = await this.$Ajax.get(
        '/api/getcomments/' + this.$route.params.id + '?pageindex=' + this.pageSize
      )
      if (res.status !== 0) return this.$message.error('请求服务器失败')
    ...
    async addComment() {
      if (this.content.trim() === '') return this.$message.error('留言不能为空')
