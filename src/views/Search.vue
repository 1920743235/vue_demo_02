  ...
  <div>
    ...
    <div v-if="isShow" style="width:100%;">
      <van-swipe-cell v-for="item in searchlist" :key="item.id">
        ...
        <template slot="right">
          <van-button @click="delGoods(item.id)" square type="danger" text="删除" />
        </template>
      </van-swipe-cell>
    </div>
    <div v-else class="searchHistory">
      ...
      <div class="tagList" style="padding:5px 10px;">
        <van-tag @click="value=item" style="margin:0 5px;" v-for="item in searchHistory" :key='item.id' size="medium">{{item}}</van-tag>
        <van-divider v-if="searchHistory.length==0" :style="{ color: '#333', borderColor: '#ccc', padding: '0 16px' }">
  暂无搜索历史
</van-divider>
      </div>
    </div>
...
export default {
  ...
  methods: {
    async onSearch() {
      ...
      // this.$router.push('/search/list?query='+this.value)
      const { data: res } = await this.$Ajax.get('/api/getprodlist')
      console.log(res)
    ...
    delGoods (id) {
      this.$dialog.confirm({
        ...
        var { data: res } = await this.$Ajax.get('/api/delproduct/' + id)
        if (res.status === 0){
          const { data: res } = await this.$Ajax.get('/api/getprodlist')
          this.searchlist = res.message
          this.$notify({
